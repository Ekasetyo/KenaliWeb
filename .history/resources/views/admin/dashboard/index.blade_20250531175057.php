...
### **Penjelasan**
- **Kode Dipadatkan:** Skrip JavaScript disederhanakan dengan format satu baris untuk mengurangi kekacauan, sambil mempertahankan fungsionalitas.
- **Penanganan Data Kosong:** Tetap menggunakan `@if (count($deteksisPerUserArray) > 0)` dan `if (deteksisPerUser.length > 0)` untuk menghindari error jika data tidak ada.
- **Konsistensi:** Pastikan semua chart menggunakan data yang dikirim dari controller (terutama `$deteksisPerUserArray` untuk "Deteksi per User").

### **Langkah Verifikasi**
- Jalankan aplikasi dan periksa dashboard.
- Periksa log di `storage/logs/laravel.log` untuk memastikan `$deteksisPerUserArray` terisi.
- Jika chart "Deteksi per User" tidak muncul, tambahkan data dummy di MongoDB:
  ```
  use kenali
  db.hasil_deteksi.insertOne({"user_id": "6829d77e9a57468c54072513", "sex": 1, "age": 18, "hypertension": 1, "heart_disease": 1, "ever_married": 1, "work_type": 2, "Residence_type": 1, "avg_glucose_level": 128, "bmi": 35, "smoking_status": 1, "prediction": "anda beresiko terkena stroke", "created_at": "2025-05-18T21:03:05.624127"})
  ```

Beri tahu saya jika masih ada masalah!