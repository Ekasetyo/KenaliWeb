...
class DashboardAdminController extends Controller
{
    public function index()
    {
        ...
        $deteksiCount = Deteksi::raw(function ($collection) {
            return $collection->aggregate([
                [
                    '$match' => [
                        'created_at' => [
                            '$gte' => new UTCDateTime(strtotime(date('Y-m-01')) * 1000), // Tanggal pertama bulan ini
                            '$lt' => new UTCDateTime(strtotime(date('Y-m-t 23:59:59')) * 1000 + 1000), // Tanggal terakhir bulan ini
                        ],
                    ],
                ],
                [
                    '$count' => 'total', // Hitung total dokumen
                ],
            ]);
        });

        // Buat array bulan default (12 bulan)
        ...
        // Buat array konsultasi per bulan
