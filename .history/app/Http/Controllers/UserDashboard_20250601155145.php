    ...
    private function getAgeRiskData()
{
    ...
    // Inisialisasi array untuk menghitung jumlah pasien per usia
    $ageCounts = array_fill(0, 101, 0); // Usia dari 0 sampai 100
    $strokeCounts = array_fill(0, 101, 0); // Jumlah stroke per usia

    foreach ($data as $item) {
        $age = $item->age;
        // Pastikan usia dalam rentang 0-100
        if ($age >= 0 && $age <= 100) {
            $ageCounts[$age]++;
