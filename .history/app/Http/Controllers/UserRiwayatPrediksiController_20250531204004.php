...
class UserRiwayatPrediksiController extends Controller
{
    public function dataPrediksi(Request $request)
    {
        ...
        $userId = (string)$user['id']; // Gunakan sebagai string
        ...
        $dataCursor = DB::connection('mongodb')
            ...
            ->find($query);
        $dataArray = iterator_to_array(new \IteratorIterator($dataCursor));
        ...
        $usersCursor = DB::connection('mongodb')
            ...
            ->find(['_id' => $userId]);
        $usersArray = iterator_to_array(new \IteratorIterator($usersCursor));
